...
for PROBLEM in PROBLEMS:
    ...
    for mode in MODES:
        ...
        for run in RUNS:
            ...
            tbh_path = os.path.join(run_path,'TPOT-BO-H',mode)
            f_tbh_prog = os.path.join(tbh_path,'TPOT-BO-H.progress')
            f_tbh_pipes = os.path.join(tbh_path,'TPOT-BO-H.pipes')
            
            tbhs_path = os.path.join(run_path,'TPOT-BO-H','sequential')
            f_tbhs_prog = os.path.join(tbhs_path,'TPOT-BO-Hs.progress')
            f_tbhs_pipes = os.path.join(tbhs_path,'TPOT-BO-Hs.pipes')
            
            check_files = [f_tb_prog, f_tb_pipes, f_tbs_prog, f_tbs_pipes, f_tbh_prog, f_tbh_pipes, f_tbhs_prog, f_tbhs_pipes]
            ...
            with open(f_tbhs_prog, 'r') as f:
                for line in f:
                    if 'AFTER' in line and 'TPOT-BO-Hs' in line:
                        next(f)
                        cv_line = next(f)
                        raw_data[run]['TPOT-BO-Hs'] = -float(cv_line.split(":")[-1])
        ...
        data['TPOT-BO-Hs'] = [v['TPOT-BO-Hs'] for k,v in raw_data.items()]
        ...
        print(f"{str(''):>{PRINT_COL}}{str('init TPOT'):>{PRINT_COL}}{str('TPOT-BASE'):>{PRINT_COL}}{str('TPOT-BO-S'):>{PRINT_COL}}{str('TPOT-BO-H'):>{PRINT_COL}}{str('TPOT-BO-Hs'):>{PRINT_COL}}")
