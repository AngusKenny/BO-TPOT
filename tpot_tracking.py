...
for problem in PROBLEMS:
    ...
    for seed_inc in range(nRUNS):
        seed = SEED + seed_inc
        track_path = os.path.join(cwd,OUT_PATH,"Tracking2",f"{problem}_{seed}_pop_{POP_SIZE}")
        ...
        for gen in range(nGENS):
            ...
            with open(f_gen, 'w') as f:
                ...
                for ind in tpot._pop:
                    ...
                    f.write(f"{list(tpot.evaluated_individuals_.keys()).index(ind_str)};{g_keys.index(grp_str)};{tpot.evaluated_individuals_[ind_str]['internal_cv_score']};{tpot.evaluated_individuals_[ind_str]['operator_count']};{n_hps};0\n")
            # fit tpot object
            ...
            with open(f_gen, 'a') as f:
                ...
                for ind in tpot._pop:
                    ...
                    grp_str = u.string_to_bracket(ind_str)
                    n_hps = len(u.string_to_params(ind_str,config_dict=default_tpot_config_dict))
                    f.write(f"{list(tpot.evaluated_individuals_.keys()).index(ind_str)};{g_keys.index(grp_str)};{tpot.evaluated_individuals_[ind_str]['internal_cv_score']};{tpot.evaluated_individuals_[ind_str]['operator_count']};{n_hps};1\n")
        ...
        with open(f_pipe, 'w') as f:
            for i,(p,v) in enumerate(tpot.evaluated_individuals_.items()):
                f.write(f"{i};{p};{v['group']};{v['internal_cv_score']};{v['operator_count']};{len(u.string_to_params(p,config_dict=default_tpot_config_dict))}\n")
