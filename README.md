...
1. _Post hoc_ methods. These are applied after an initial TPOT search (typically, 80 generations) has completed. They include:
    ...
    - `TPOT-BO-O`, which operates similarly to `TPOT-BO-H`, however BO budget is allocated according to OCBA principles, reducing the number of candidates in each subsequent generation.  
&nbsp;
