...
One final element that should be mentioned in this digression is the `deap.gp.PrimitiveSetTyped` class. This class contains a list of all of the `Primitive` objects, and their related `Terminal` objects that can be used by DEAP to solve a GP problem - as well as all of the possible values each `Terminal` object can take. This "master list" is maintained in the attribute `tpot._pset` and it is referenced any time an individual in the population is evaluated. At its core, TPOT is a grid-search algorithm, 
