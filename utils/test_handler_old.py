...
class TestHandler(object):
        ...
        for prob in self.prob_list:
            ...
            for run in self.run_list:
                ...
                if self.params['RUN_oTPOT-BASE']: 
                    rmtree(os.path.join(run_path,"oTPOT-BASE"),ignore_errors=True)
        self.vprint.v0("Done!\n")
    ...
    def run_oTPOT_BASE(self):
        run = self.run_path.split("_")[-1] 
        otb_dir = f"oTPOT-BASE"
        otb_path = os.path.join(self.run_path,otb_dir)
        
        if not os.path.exists(otb_path):
            os.makedirs(otb_path)
        
        try:
            t_tpot_start = time.time()
            self.vprint.v1(f"{u.CYAN_U}****** Generating oTPOT data (run {run}) for problem '{self.problem}' ******{u.OFF}\n")
            ...
            with open(self.fname_prog, 'a') as f:
                f.write(f"({time.strftime('%d %b, %H:%M', time.localtime())}) - {self.problem} - oTPOT-BASE (run {run}): {res_txt} ({round(t_tpot_end-t_tpot_start,2)}s)\n")
        except:
            trace = traceback.format_exc()
            self.vprint.verr(f"FAILED:\n{trace}")
            with open(self.fname_prog, 'a') as f:
                f.write(f"({time.strftime('%d %b, %H:%M', time.localtime())}) - {self.problem} - oTPOT-BASE (run {run}): Failed..\n{trace}\n\n")
            return None
